...
var updateQuantity = function (evt) {
  ...
  var amount2 = config.unitAmount2;
  ...
  amount2 = zeroDecimalCurrency ? amount2 : amount2 / 100;

  var total = (quantity * amount).toFixed(2);
  var total2 = (quantity * amount2).toFixed(2);

  var formattedTotal = numberFormat.format(total);
  var formattedTotal2 = numberFormat.format(total2);
  ...
  document
    ...
    document
    .getElementById('nobike')
    .setAttribute('i18n-options', `{ "total": "${formattedTotal2}" }`);

  updateContent('button.submit');
  updateContent('button.nobike');
...
var createCheckoutSession = function () {
  ...
  var inputPr = document.getElementById('price');
  var option = parseInt(inputPr.value);    
...
fetch('/config')
  ...
  .then(function (json) {
    ...
    document.querySelector('#nobike').addEventListener('click', function (evt) {
